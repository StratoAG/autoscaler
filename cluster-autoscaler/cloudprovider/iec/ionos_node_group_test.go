...
func initNodeGroup(
	...
	return testNodeGroup(client, initK8sNodePool(size, min, max, "123", profitbricks.StateAvailable))
}

func initK8sNodePool(nodecount, min, max uint32, id, state string) *profitbricks.KubernetesNodePool{
...
func TestNodeGroup_IncreaseSize(t *testing.T) {
	...
	t.Run("failure, update kubernetes nodepool failed", func(t *testing.T) {
		delta := 1
		expectedErr := errors.New("oops, something went wrong")
		client := mocks.Client{}
		client.On("UpdateKubernetesNodePool", mock.Anything, mock.Anything, mock.Anything).Return(
			nil, expectedErr).Once()
		ng := initNodeGroup(2, 1, 3, &client)
	...
	t.Run("failure, failed to finally get the nodepool", func(t *testing.T) {
		...
		client.On("GetKubernetesNodePool", ng.clusterID, ng.nodePool.ID).
			Return(updated, nil).Once()
		client.On("GetKubernetesNodePool", ng.clusterID, ng.nodePool.ID).
			Return(nil, errors.New("oops, something went wrong")).Once()
...
func TestNodeGroup_DecreaseTargetSize(t *testing.T) {
	...
	t.Run("success, decreased to minimum", func(t *testing.T) {
		delta := -2
		numberOfNodes := uint32(3)
		client := mocks.Client{}
		ng := initNodeGroup(numberOfNodes, 1, 3, &client)
	...
	t.Run("failure, updated nodepool size does not match targetsize", func(t *testing.T) {
		delta := -1
		client := mocks.Client{}
		client.On("UpdateKubernetesNodePool", mock.Anything, mock.Anything, mock.Anything).Return(
			initK8sNodePool(2, 1, 3, "123", profitbricks.StateAvailable), nil).Once()
		ng := initNodeGroup(2, 1, 3, &client)

		err := ng.DecreaseTargetSize(delta)
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "couldn't decrease size to")
		client.AssertExpectations(t)
	})
...
func TestNodeGroup_DeleteNodes(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		client.On("GetKubernetesNodePool", ng.clusterID, ng.nodePool.ID).
			Return(initK8sNodePool(count, min, max, id, state), nil).Times(5)
		client.On("GetKubernetesNodePool", ng.clusterID, ng.nodePool.ID).
			Return(initK8sNodePool(count-1, min, max, id, state), nil).Once()
		// Delete second node
		client.On("DeleteKubernetesNode", ng.clusterID, ng.id, "2").Return(&http.Header{}, nil).Once()
		// Poll 5 times before success
		client.On("GetKubernetesNodePool", ng.clusterID, ng.nodePool.ID).
			Return(initK8sNodePool(count-1, min, max, id, state), nil).Times(5)
		client.On("GetKubernetesNodePool", ng.clusterID, ng.nodePool.ID).
			Return(initK8sNodePool(count-2, min, max, id, state), nil).Once()
	...
	t.Run("client node deletion fails", func(t *testing.T) {
		...
		// Poll 5 times before success
		client.On("GetKubernetesNodePool", ng.clusterID, ng.nodePool.ID).
			Return(initK8sNodePool(count, min, max, id, state), nil).Times(4)
		client.On("GetKubernetesNodePool", ng.clusterID, ng.nodePool.ID).
			Return(initK8sNodePool(count-1, min, max, id, state), nil).Once()
		// Fail on second node
		...
		assert.Error(t, err)
		assert.Contains(t, err.Error(),"deleting node failed for cluster:", "wrong error")
		client.AssertExpectations(t)
	})

	t.Run("client node deletion times out", func(t *testing.T) {
		ng := initNodeGroup(3, 1, 3, &client)
		// Delete first node
		client.On("DeleteKubernetesNode", ng.clusterID, ng.id, "1").Return(&http.Header{}, nil).Once()
		// Poll never succeeds
		client.On("GetKubernetesNodePool", ng.clusterID, ng.nodePool.ID).
		...
		err := ng.DeleteNodes(nodes)
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "failed to wait for nodepool update: timed out waiting for the condition")
		client.AssertExpectations(t)
	})
}

func TestNodeGroup_Nodes(t *testing.T) {
	numberOfNodes := uint32(4)
	ionosclient := mocks.Client{}
...
func TestNodeGroup_Exist(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		numberOfNodes := uint32(3)
		ng := initNodeGroup(numberOfNodes,1, 3, &ionosclient)
		exist := ng.Exist()
