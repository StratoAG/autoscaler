...
// IECloudProvider implements CloudProvider interface.
type IECloudProvider struct {
	manager         *Manager
	resourceLimiter *cloudprovider.ResourceLimiter
}

func BuildIECloudProvider(manager *Manager, rl *cloudprovider.ResourceLimiter) (*IECloudProvider, error) {
	return &IECloudProvider{
		manager:         manager,
		resourceLimiter: rl,
	}, nil
}
...
func (d *IECloudProvider) Name() string {
	return cloudprovider.IECProviderName
}

// NodeGroups returns all node groups configured for this cloud provider.
func (d *IECloudProvider) NodeGroups() []cloudprovider.NodeGroup {
...
func (d *IECloudProvider) NodeGroupForNode(node *apiv1.Node) (cloudprovider.NodeGroup, error) {
...
func (d *IECloudProvider) Pricing() (cloudprovider.PricingModel, errors.AutoscalerError) {
...
func (d *IECloudProvider) GetAvailableMachineTypes() ([]string, error) {
	return []string{}, nil
}
...
func (d *IECloudProvider) NewNodeGroup(
	machineType string,
...
func (d *IECloudProvider) GetResourceLimiter() (*cloudprovider.ResourceLimiter, error) {
	return d.resourceLimiter, nil
}

// GPULabel returns the label added to nodes with GPU resource.
func (d *IECloudProvider) GPULabel() string {
	return GPULabel
}

// GetAvailableGPUTypes return all available GPU types cloud provider supports.
func (d *IECloudProvider) GetAvailableGPUTypes() map[string]struct{} {
	return nil
}

// Cleanup cleans up open resources before the cloud provider is destroyed,
// i.e. go routines etc.
func (d *IECloudProvider) Cleanup() error {
	return nil
}
...
func (d *IECloudProvider) Refresh() error {
	klog.V(4).Info("Refreshing node group cache")
	return d.manager.Refresh()
}

// BuildIonosEnterprise builds the Ionos Enterprise cloud provider.
func BuildIonosEnterprise(
	...
	manager, err := CreateIECManager(configFile)
	if err != nil {
		klog.Fatalf("Failed to create Ionos Enterprise manager: %v", err)
	}
